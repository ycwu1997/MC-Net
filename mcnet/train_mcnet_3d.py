...
if __name__ == "__main__":
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            loss = args.beta * loss_seg_dice + consistency_weight * loss_consist
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
...
#                     seg_out = np.concatenate((np.expand_dims(seg_out1,axis=0),  np.expand_dims(seg_out2,axis=0) ),axis=0)
#                     seg_mean = np.mean(seg_out, axis = 0)
#                     seg_std = np.std(seg_out, axis = 0)
